...
## Sprint 14 Advance Plan

61 items total (49 Sprint 13 deferrals + 12 new improvements). Too large for one sprint.

Recommended split:
- **Sprint 14A (~3 weeks):** EventBus full architecture + backend restructure + D-103 + legacy removal + security keys
- **Sprint 14B (~3 weeks):** Frontend restructure + tooling + monorepo + Docker + coverage + docs

Key new items not in any previous plan:
- N1: Structured logging standard
- N5: Telegram Bot unit tests (currently zero)
- N6: WMCP tool inventory test (18 tools, no automated check)
- N7-N8: Hardcoded API key + sourceUserId removal
- N9: Dependency vulnerability audit
- N11: Mission error recovery (graceful stage failure)

See SPRINT-14-ADVANCE-PLAN.md for full detail.
